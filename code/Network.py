...
class ResNet(object):
	...
	def _batch_norm_relu(self, inputs, training):
		...
		### YOUR CODE HERE
		batch_normalization = tf.layers.batch_normalization(inputs=inputs, momentum=0.9, training=training)
		outputs = tf.nn.relu(batch_normalization)
		### END CODE HERE
	...
	def _output_layer(self, inputs, training):
		...
		### YOUR CODE HERE
		#We have an average pooling layer, followed by a fully connected layer, followed by a softmax layer at output
		outputs = tf.layers.average_pooling2d(inputs=inputs, pool_size=(8,8), strides=1)
		#Flatten the tensor now
	...
	def _stack_layer(self, inputs, filters, block_fn, strides, training):
		...
		def projection_shortcut(inputs):
			### YOUR CODE HERE
			return tf.layers.conv2d(inputs=inputs, filters=filters_out, kernel_size=1, strides=strides)
			### END CODE HERE
	...
	def _standard_block_v1(self, inputs, filters, training, projection_shortcut, strides):
		...
		#For standard block, we have Conv2d, followed by BN, RELU, Conv2d, BN, addition, then Relu
	...
	def _bottleneck_block_v2(self, inputs, filters, training, projection_shortcut, strides):
		...
		### YOUR CODE HERE
		#Do batch normalization and relu as the first step to the input
		shortcut = inputs
		inputs = self._batch_norm_relu(inputs, training)
		if projection_shortcut is not None:
			shortcut = projection_shortcut(inputs)

		#1x1 conv, 3x3 conv, then again 1x1 conv with BN and Relu in between
