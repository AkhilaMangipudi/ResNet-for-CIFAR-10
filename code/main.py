...
def configure():
	...
	flags.DEFINE_integer('resnet_size', 9, 'n: the size of ResNet-(6n+2) v1 or ResNet-(9n+2) v2')
...
def main(_):
	...
	# epochs = [200,300]
	# for model.conf.resnet_version in [1,2]:
	#	for model.conf.weight_decay in [2e-4,0.0001]:
	# 		for model.conf.batch_size in [32,128,256]:
	# 			for epoch in epochs:
	#  				print(model.conf.resnet_version, model.conf.weight_decay, model.conf.weight_decay, epoch)
	#  				model.train(x_train_new, y_train_new, epoch)
	#  				model.test_or_validate(x_valid, y_valid, [epoch-40, epoch-30, epoch-20, epoch-10, epoch])
	#model.train(x_train, y_train, 300)

	# Third step: after re-training, test your model on the test set.
	# Report testing accuracy in your hard-copy report.
	#model.test_or_validate(x_test, y_test, [300])
	### END CODE HERE
