...
class Cifar(object):
	...
	def setup(self, training):
		...
		if training:
			...
			### YOUR CODE HERE
			# cross entropy
			cross_entropy_loss = tf.losses.sparse_softmax_cross_entropy(logits = logits, labels=self.labels, reduction=tf.losses.Reduction.MEAN)
			# final loss function
	...
	def train(self, x_train, y_train, max_epoch):
		...
		for epoch in range(1, max_epoch+1):
			...
			for i in range(num_batches):
				...
				x_batch = curr_x_train[i*self.conf.batch_size:(i+1)*self.conf.batch_size]
				x_batch_temp = []
				for image in x_batch:
					x_batch_temp.append(parse_record(image, True))
				x_batch = np.asarray(x_batch_temp)
				y_batch = curr_y_train[i*self.conf.batch_size:(i+1)*self.conf.batch_size]
				### END CODE HERE
	...
	def test_or_validate(self, x, y, checkpoint_num_list):
		...
		for checkpoint_num in checkpoint_num_list:
			...
			for i in tqdm(range(x.shape[0])):
				### YOUR CODE HERE
				feed_dict = {self.inputs: parse_record(x[i], False).reshape((1, 32, 32, 3)), self.labels: y[i]}
